...
export default function FlowerScene({ onSelect, onFocusComplete, onStatus, paused, modalOpen, visited, galleryUnlocked, focusMemoryId, nextMemoryId, resetKey, reducedMotion, debug }) {
  const host = useRef(null), live = useRef({})
  live.current = { paused, modalOpen, visited, galleryUnlocked, focusMemoryId, nextMemoryId, resetKey, reducedMotion, debug, onSelect, onFocusComplete, onStatus }
  useEffect(() => {
    ...
    const paleGold = new THREE.MeshStandardMaterial({ color:'#fff0a0', vertexColors: true, map: paint, roughness: .88, metalness: 0, emissive:'#8d6100', emissiveIntensity:.075 })
    ...
    const cream = new THREE.MeshStandardMaterial({ color: '#f1c43a', roughness: .93 })
    ...
    const galleryLamp = new THREE.Group(); galleryLamp.position.set(-.06,1.24,.05); crown.add(galleryLamp)
    const galleryBulb = new THREE.Mesh(new THREE.SphereGeometry(.072,18,14),new THREE.MeshBasicMaterial({color:new THREE.Color(1.8,1.55,0.95),toneMapped:false})); galleryLamp.add(galleryBulb)
    const galleryHalo = new THREE.Sprite(new THREE.SpriteMaterial({map:glowTexture,transparent:true,opacity:0,depthWrite:false,blending:THREE.AdditiveBlending,toneMapped:false}))
    galleryHalo.scale.setScalar(1.18); galleryLamp.add(galleryHalo)
    const galleryHit = new THREE.Mesh(new THREE.SphereGeometry(.34,14,10),new THREE.MeshBasicMaterial({transparent:true,opacity:0,depthWrite:false}))
    galleryHit.userData.memoryId='gallery'; galleryLamp.add(galleryHit); targets.push(galleryHit)
    ...
    function pick(event){
      ...
      // The unlocked central gallery light gets priority over the stigma/cup geometry.
      if(opts.galleryUnlocked){
        const galleryHitResult=raycaster.intersectObject(galleryHit,false)[0]
        if(galleryHitResult) return 'gallery'
      }

      const hit=raycaster.intersectObjects([...targets,...solids],false)[0]
      const id=hit?.object.userData.memoryId??null
      if(!id) return null
      const allowed=id==='gallery' ? opts.galleryUnlocked : (opts.visited.has(id)||opts.nextMemoryId===id||opts.visited.size===6)
    ...
    function pointerUp(event){
      ...
      const id=pick(event)
      if(id){ (id==='gallery'?galleryLamp:lamps[id-1].lamp).getWorldPosition(burst.position);delete burst.userData.wallStart;burst.visible=true;live.current.onSelect?.(id) }
    }
    function pointerLeave(){hovered=null;down=null;container.style.cursor='grab'}
    function focusPetal(id){
      const entry=id==='gallery' ? { lamp: galleryLamp } : lamps[id-1]
      if(!entry) return
      const lampPos=new THREE.Vector3(); entry.lamp.getWorldPosition(lampPos)

      // Move to a canonical "front of the petal" view.
      // The camera shifts to the outward-facing side of the selected petal,
      // while the orbit target stays flower-biased so the user can still move around the flower afterwards.
      const fromPos=camera.position.clone()
      const fromTarget=controls.target.clone()
      const flowerCenter=new THREE.Vector3(0,2.1,0)

      const frontDir=lampPos.clone().sub(flowerCenter)
      frontDir.y*=0.12
      if(frontDir.lengthSq()<1e-5) frontDir.set(0,0,1)
      frontDir.normalize()

      const centerToPetal=flowerCenter.distanceTo(lampPos)
      const desiredTarget=flowerCenter.clone().lerp(lampPos,0.26)
      const desiredDistance=Math.max(5.0, Math.min(5.8, centerToPetal + 3.15))
      const toPos=lampPos.clone()
        .add(frontDir.clone().multiplyScalar(desiredDistance))
        .add(new THREE.Vector3(0,0.55,0))

      toPos.y=Math.max(2.7,toPos.y)
      ...
      cameraTween={
        ...
        duration:live.current.reducedMotion ? 0.01 : 1.32
      }
    }
    ...
    function animate(now){
      ...
      const galleryActive = hovered === 'gallery'
      const galleryPulse = opts.galleryUnlocked ? (moving ? .18 + Math.max(0, Math.sin(elapsed * 2.55)) * .28 : .22) : 0
      galleryHalo.visible = opts.galleryUnlocked
      galleryBulb.visible = opts.galleryUnlocked
      galleryHalo.scale.setScalar((galleryActive ? 1.48 : 1.24) + galleryPulse)
      galleryHalo.material.opacity = opts.galleryUnlocked ? (galleryActive ? 1 : 0.96) : 0
      galleryBulb.scale.setScalar(opts.galleryUnlocked ? (galleryActive ? 1.34 : 1.14) : 0.92)
      galleryBulb.material.color.setRGB(opts.galleryUnlocked ? 3.2 : 0.35, opts.galleryUnlocked ? 2.5 : 0.31, opts.galleryUnlocked ? 1.18 : 0.24)
      if(burst.visible){
